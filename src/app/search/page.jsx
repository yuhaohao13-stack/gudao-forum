...
function Results() {
  ...
  return (
    <div className="anim-fade-in">
      <h1 className="text-2xl font-bold font-serif text-[#1a1a1a] mb-5">🔍 搜索</h1>
      <form onSubmit={e => { e.preventDefault(); if (input.trim()) router.push(`/search?q=${encodeURIComponent(input.trim())}`) }} className="mb-6 flex gap-2">
        <div className="relative flex-1">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#ccc] text-sm">🔍</span>
      ...
      {loading && <div className="flex justify-center py-8"><div className="w-5 h-5 border-2 border-[#c23531]/30 border-t-[#c23531] rounded-full animate-spin" /></div>}
      {!loading && searched && query && (
        <><p className="text-sm text-[#999] mb-4">搜索「{query}」— {results.length} 个结果</p>
        <div className="space-y-2.5">
          {results.length === 0 ? (
            <div className="card p-10 text-center"><div className="text-3xl mb-2">🔍</div><p className="text-[#999]">没有找到相关帖子</p></div>
          ) : results.map((t, i) => (
              ...
              <div className="text-[#1a1a1a]">
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="text-xs text-[#999] truncate min-w-0">
                    ...
                    {t.categories && <><span className="text-[#d8d0c0] mx-1.5">/</span>{t.categories?.name}</>}
                  </div>
      ...
      {!searched && !query && <div className="text-center py-16"><div className="text-3xl mb-2">🔍</div><p className="text-[#999]">输入关键词，搜索全站帖子</p></div>}
    </div>
  )
}

export default function SearchPage() {
  return <Suspense fallback={<div className="flex justify-center py-20"><div className="w-5 h-5 border-2 border-[#c23531]/30 border-t-[#c23531] rounded-full animate-spin" /></div>}><Results /></Suspense>
}
