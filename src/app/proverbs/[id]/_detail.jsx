...
export default function ProverbDetailPage() {
  ...
  const [showGoldLock, setShowGoldLock] = useState(false)
    ...
    return (
      <div className="anim-fade-in max-w-2xl mx-auto">
        ...
        <div className="bg-white border border-[#ece8e0] rounded-xl px-5 py-12 sm:px-8 text-center">
          ...
          {/* SEO 正文预览：未登录也展示释义与故事开头（服务端渲染可见） */}
          <div className="relative mb-6 text-left overflow-hidden max-h-56">
            <div className="space-y-3">
              <p className="text-sm leading-relaxed text-[#2a2a2a]"><span className="text-[#b45309] font-medium">【释义】</span>{proverb.meaning}</p>
              <p className="text-sm leading-relaxed text-[#555]">{proverb.story.slice(0, 400)}{proverb.story.length > 400 ? '……' : ''}</p>
            </div>
    ...
    return (
      <div className="anim-fade-in max-w-2xl mx-auto">
        ...
        <div className="bg-white border border-[#ece8e0] rounded-xl px-5 py-6 sm:px-8 sm:py-8 mb-4">
          <h1 className="text-xl sm:text-2xl font-bold text-center text-[#1a1a1a] tracking-wide mb-3">{proverb.proverb}</h1>
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          {prevProverb ? (
            <Link href={`/proverbs/${prevProverb.id}`}
          ...
          {nextProverb ? (
            <Link href={`/proverbs/${nextProverb.id}`}
        ...
        <div className="text-center">
          <Link href="/proverbs" className="inline-flex items-center gap-1 text-[11px] text-[#b45309] hover:underline transition-colors">← 返回全部谚语</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="anim-fade-in max-w-2xl mx-auto">
      ...
      <GoldLock previewLines={5}>
        <div className="bg-white border border-[#ece8e0] rounded-xl px-5 py-6 sm:px-8 sm:py-8 mb-4">
          ...
          <div className="relative">
            <Quote size={18} className="text-[#ece8e0] absolute -top-1 -left-1 rotate-180" />
            <p className="text-sm leading-relaxed text-[#2a2a2a] px-6 py-1 indent-0">{proverb.story}</p>
            <Quote size={18} className="text-[#ece8e0] absolute -bottom-1 -right-1" />
          </div>
        </div>
      </GoldLock>
