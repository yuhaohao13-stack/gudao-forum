...
export default function GeminiChatPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        if (data.reason === 'exhausted') { setLockInfo({ allowed: false, reason: 'exhausted' }); setMessages(newMessages); return }
  ...
  if (authLoading) return <div className="flex justify-center py-20"><Loader2 size={28} className="animate-spin text-[#059669]" /></div>

  if (lockInfo) {
    return (
      <div className="max-w-3xl mx-auto py-6 px-3">
        <Link href="/ai-tools" className="text-xs text-[#059669] hover:underline inline-flex items-center gap-1 mb-4">
    ...
    <div className="max-w-3xl mx-auto py-4 sm:py-6 px-3 sm:px-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-[10px] text-[#999]">
          剩余 <span className="font-bold text-[#059669]">{remaining}</span> / {max} 次
        </div>
      </div>
      ...
      <div className="bg-white border border-[#ece8e0] rounded-xl overflow-hidden flex flex-col" style={{ height: '65vh' }}>
        <div className="bg-gradient-to-r from-[#ecfdf5] to-[#d1fae5] px-4 py-2.5 border-b border-[#a7f3d0]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-[#059669]">Gemini 3.6 Flash — 快速响应模型</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
          {messages.length === 0 && (
            <div className="text-center py-12 text-xs text-[#bbb]">
              ...
              <p>输入您的问题，开始与 Gemini 对话</p>
            </div>
          )}
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-2 ${m.role === 'user' ? 'justify-end' : ''}`}>
              {m.role === 'assistant' && <Bot size={20} className="shrink-0 mt-1 text-[#059669]" />}
              <div className={`text-xs sm:text-sm px-3 py-2 rounded-xl max-w-[85%] whitespace-pre-wrap ${
                m.role === 'user' ? 'bg-[#059669] text-white rounded-br-sm' : 'bg-[#f5f5f5] text-[#1a1a1a] rounded-bl-sm'
              }`}>{m.content}</div>
              {m.role === 'user' && <User size={20} className="shrink-0 mt-1 text-[#059669]" />}
            </div>
          ))}
          {sending && (
            <div className="flex gap-2">
              <Bot size={20} className="shrink-0 mt-1 text-[#059669]" />
