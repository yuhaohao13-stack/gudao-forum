...
const TECH_BRAND_MAP = {
  ...
  'OPPO': { en: 'OPPO', label: 'OPPO' },
  'vivo': { en: 'vivo', label: 'vivo' },
  '一加': { en: 'OnePlus', label: '一加 OnePlus' },
  '荣耀': { en: 'Honor', label: '荣耀 Honor' },
  '摩托罗拉': { en: 'Motorola', label: '摩托罗拉 Motorola' },
  '红魔': { en: 'RedMagic', label: '红魔 RedMagic' },
  '夏普': { en: 'Sharp', label: 'Sharp' },
  '华硕': { en: 'ASUS', label: '华硕 ASUS' },
  '其他安卓': { en: 'Other Android', label: '其他安卓 Other' },
  '电脑主板': { en: 'PC', label: '电脑主板 PC' },
  '游戏机': { en: 'Console', label: '游戏机 Console' },
  '相机': { en: 'Camera', label: '相机 Camera' },
  '手表': { en: 'Watch', label: '手表 Watch' },
  '耳机': { en: 'Headphones', label: '耳机 Headphones' },
  'Kobo 电子书': { en: 'Kobo', label: 'Kobo 电子书' },
...
export default function ThreadDetail({ initial }) {
  const { thread, content: initialContent, replies: initialReplies, likeCount: initialLikeCount, view: initialView, lockReason: initialLockReason } = initial
  const id = thread.id
  const { user, profile } = useAuth()
  const [content, setContent] = useState(initialContent)
  const [replies, setReplies] = useState(initialReplies || [])
  const [view, setView] = useState(initialView)         // 'full' | 'locked'
  const [lockReason, setLockReason] = useState(initialLockReason || 'login')
  ...
  // 浏览量 +1（客户端执行，爬虫不带 JS 不会刷量）
  useEffect(() => {
    if (incremented.current) return
    incremented.current = true
    const prev = thread.view_count || 0
    setViewCount(prev + 1)
    supabase.from('threads').update({ view_count: prev + 1 }).eq('id', id)
      .then(() => {}).catch(() => {})
  }, [id, thread.view_count, supabase])
  ...
  // 兜底：SSR 判定为锁定，但客户端会话其实是钻石会员（cookie 过期/多标签页场景）
  // → 走原 /api/thread-content 鉴权接口补拉全文（XHR 已验证比 fetch 稳）
  useEffect(() => {
    if (view !== 'locked') return
    if (!user || profile?.membership_level !== 'diamond') return
    let cancelled = false
    const loadFull = async () => {
      try {
        ...
        if (data.content !== undefined) {
          ...
          setView('full')
          supabase.from('replies').select('*, profiles(username, display_name)').eq('thread_id', id).order('created_at')
            .then(({ data: r }) => { if (r && !cancelled) setReplies(r) })
        }
      } catch (e) {
  ...
  }, [view, user, profile?.membership_level, id, supabase])
  ...
  const handleReply = async (e) => {
    ...
    if (err) { setError(err.message) } else {
      ...
      // 与 SSR 传入的回复形状保持一致（profiles → author）
      setReplies((r || []).map(x => ({
        id: x.id, author_id: x.author_id, content: x.content, is_deleted: x.is_deleted,
        created_at: x.created_at, createdAtText: '',
        author: x.profiles ? { username: x.profiles.username, display_name: x.profiles.display_name } : null,
      })))
      setReplyContent('')
    }
    setSending(false)
  ...
  const authorName = thread.author?.display_name || thread.author?.username || '匿名'
    ...
    <div className="anim-fade-in w-full sm:max-w-3xl sm:mx-auto">
      ...
      <article className="mt-4 card p-6 sm:p-8 anim-up">
        ...
        <div className="flex items-center gap-2 mt-3 text-sm text-[#aaa]">
          <Link href={`/profile/${thread.author.id}`} className="w-6 h-6 rounded-full bg-[#c23531] flex items-center justify-center text-[10px] text-white font-bold">
            {authorInitial}
          </Link>
        ...
        {isLocked ? (
          <>
            {content && (
              <>
                <div className="text-[#444] leading-7 sm:leading-8 whitespace-pre-wrap text-sm sm:text-base font-mono opacity-90">
                  {renderContent(content)}
                </div>
                <div className="mt-2 text-[#bbb] text-xs select-none">… 摘录至此，完整案例仅限钻石会员</div>
                <div className="my-6 h-px bg-[#f0f0f0]" />
              </>
            )}
          ...
          <>
            {content?.includes('douyin.com') && (() => {
              const m = content.match(/https:\/\/(?:www\.douyin\.com\/video\/\d+|v\.douyin\.com\/[\w\/]+)/)
            ...
            <div className="text-[#444] leading-7 sm:leading-8 whitespace-pre-wrap text-sm sm:text-base font-mono">{renderContent(content)}</div>
                    ...
                    <>
                      <div className="flex items-center gap-2 text-sm text-[#aaa] mb-2">
                        <Link href={`/profile/${r.author_id}`} className="w-5 h-5 rounded-full bg-[#ddd] flex items-center justify-center text-[8px] text-white font-bold">
                          {((r.author?.display_name || r.author?.username || '?')[0])}
                        </Link>
                        <Link href={`/profile/${r.author_id}`} className="text-[#888] hover:text-[#c23531] transition-colors">{r.author?.display_name || r.author?.username}</Link>
                        <span>·</span>
                        <span>{r.createdAtText || new Date(r.created_at).toLocaleString('zh-CN')}</span>
                        {profile?.role === 'admin' && (
