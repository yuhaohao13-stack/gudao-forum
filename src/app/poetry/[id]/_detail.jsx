    ...
    return (
      <div className="anim-fade-in max-w-2xl mx-auto">
        ...
        <div className="bg-white border border-[#ece8e0] rounded-xl px-5 py-12 sm:px-8 text-center">
          ...
          {/* SEO 正文预览：未登录也展示诗作开头（服务端渲染可见） */}
          <div className="relative mb-6 max-h-64 overflow-hidden">
            <div className="space-y-2.5 text-center">
              {poem.content.slice(0, 600).split('\n').filter((l) => l.trim()).map((line, i) => (
                <p key={i} className="text-base sm:text-lg leading-relaxed text-[#2a2a2a]"
                  style={{ fontFamily: "'Noto Serif SC', 'Source Han Serif SC', 'STSong', 'SimSun', 'Songti SC', Georgia, serif", letterSpacing: '0.05em' }}>{line}</p>
              ))}
            </div>
    ...
    return (
      <div className="anim-fade-in max-w-2xl mx-auto">
        ...
        <div className="bg-white border border-[#ece8e0] rounded-xl px-5 py-6 sm:px-8 sm:py-8 mb-4">
          <h1 className="text-xl sm:text-2xl font-bold text-center text-[#1a1a1a] tracking-wide mb-1">{poem.title}</h1>
          <p className="text-center text-sm text-[#b0a898] mb-3">—— {poem.author} ——</p>
          <div className="flex justify-center mb-5">
            <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-medium
              ${poem.category === '山水田园' ? 'bg-green-50 text-green-700' :
                ...
                'bg-amber-50 text-amber-700'}`}>{poem.category}</span>
          </div>
        ...
        <div className="text-center">
          <Link href="/poetry" className="inline-flex items-center gap-1 text-[11px] text-[#b45309] hover:underline transition-colors">← 返回全部唐诗</Link>
        </div>
      </div>
    )
  }
        ...
        <div className="bg-white border border-[#ece8e0] rounded-xl px-5 py-6 sm:px-8 sm:py-8 mb-4">
          ...
          <div className="space-y-2.5 text-center">
            {contentLines.slice(0, 3).map((line, i) => (
              <p key={i} className="text-base sm:text-lg leading-relaxed text-[#2a2a2a]">{line}</p>
            ))}
          </div>
        </div>
      </GoldLock>
