...
export default function MilitiaPage() {
  ...
  return (
    <div className="space-y-4">
      {/* 书籍标题 */}
      <div className="bg-gradient-to-r from-[#fdf8f4] to-[#f8f0e8] border border-[#eee8dc] rounded-xl p-4 sm:p-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#444] bg-opacity-10 flex items-center justify-center shrink-0">
            <BookOpen size={20} className="text-[#444]" />
          </div>
          <div>
      ...
      <div className="space-y-1.5">
        ...
        {pageChapters.map((ch, i) => (
          <Link
            key={ch.slug}
            href={`/books/militia/${ch.slug}`}
            className="block bg-white border border-[#ece8e0] rounded-xl px-4 py-3 transition-all hover:border-[#b45309] hover:shadow-sm hover:-translate-y-0.5"
          >
      ...
      <div className="flex items-center justify-between gap-3 pt-2 text-xs">
        {currentPage > 1 ? (
          <button
            ...
            className="text-[#b45309] hover:underline inline-flex items-center gap-1"
          >
