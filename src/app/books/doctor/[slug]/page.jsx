...
export default function DoctorChapterPage() {
  const params = useParams()
  const { slug: chapterSlug } = params
  const book = doctorHandbook
  const bookSlug = 'doctor'
  ...
  if (!chapter) {
    return (
      <div className="text-center py-12">
        ...
        <Link href={`/books/${bookSlug}`} className="text-[#16a34a] hover:underline text-sm mt-2 inline-block">
          返回目录
        </Link>
      </div>
    )
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-[#ece8e0] rounded-xl p-4">
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          {chapter.sections.map((s, i) => (
            <span key={i} className="text-[10px] bg-[#f0faf0] text-[#666] px-2 py-0.5 rounded-full">{s}</span>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-[#ece8e0] rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5 bg-[#fafafa] border-b border-[#ece8e0]">
          ...
          <div className="flex items-center gap-2">
            <a href={pdfViewUrl} target="_blank" rel="noopener noreferrer" className="text-[10px] text-[#16a34a] hover:underline inline-flex items-center gap-1">
              <ExternalLink size={11} /> 新窗口打开
            </a>
            <a href={book.pdfUrl} download className="text-[10px] text-[#16a34a] hover:underline inline-flex items-center gap-1">
      ...
      <div className="flex items-center justify-between gap-3">
        {prevCh ? (
          <Link href={`/books/${bookSlug}/${prevCh.slug}`} className="flex-1 bg-white border border-[#ece8e0] rounded-xl p-3 transition-all hover:border-[#16a34a] hover:shadow-sm">
        ...
        {nextCh ? (
          <Link href={`/books/${bookSlug}/${nextCh.slug}`} className="flex-1 bg-white border border-[#ece8e0] rounded-xl p-3 transition-all hover:border-[#16a34a] hover:shadow-sm text-right">
      ...
      <div className="text-center">
        <Link href={`/books/${bookSlug}`} className="text-xs text-[#16a34a] hover:underline inline-flex items-center gap-1">
